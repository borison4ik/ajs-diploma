...
export default class GameController {
  ...
  onCellClick(index) {
    ...
    if (this.checkIndex(index) && char.character.player === GameState.player) {
      ...
      // переход персонажа на пустую клетку
    } else if (
      GameState.char &&
      GameState.moveArea.includes(index) &&
      !this.checkNotPlayer(index)
    ) {
      ...
      this.changePlayer();
      this.gamePlay.redrawPositions(this.currentTeam.team);
      if (GameState.player === 'computer') this.contrAttack();
      // реализация атаки
    } else if (
      GameState.char &&
      GameState.attackArea.includes(index) &&
      this.checkNotPlayer(index)
    ) {
      ...
      this.gamePlay.showDamage(index, damage).then((responce) => {
        ...
        this.gamePlay.redrawPositions(this.currentTeam.team);
        if (
          !this.currentTeam.team.some((e) => e.character.player === 'computer')
        ) {
          this.checkWin();
        } else if (GameState.player === 'computer') this.contrAttack();
      });
      return;
    ...
    if (
      ...
      char.character.player !== GameState.player
    ) {
  ...
  onCellEnter(index) {
    ...
    if (this.checkIndex(index)) {
      ...
      const { level, attack, defence, health } = char.character;
  ...
  contrAttack() {
    ...
    this.currentTeam.team.forEach((e) => {
      if (e.character.player === 'computer') arrayOfChars.push(e);
    });
    ...
    if (ind !== undefined) {
      ...
      do {
        newPosition =
          GameState.moveArea[
            Math.floor(Math.random() * GameState.moveArea.length)
          ];
  ...
  endGame() {
    ...
    alert(
      `Победа! Вы набрали ${GameState.score} очков! \nРекорд: ${GameState.maxScore} очков`
    );
  }
  ...
  addScore() {
    ...
    this.currentTeam.team.forEach((e) => {
      ...
      if (GameState.maxScore) {
        GameState.maxScore < score
          ? (GameState.maxScore = score)
          : (GameState.maxScore = GameState.maxScore);
        return;
      }
      GameState.maxScore = score;
    });
  ...
  getChar(position) {
    return this.currentTeam.team[
      this.currentTeam.team.findIndex((char) => char.position === position)
    ];
  }

  changePlayer() {
    GameState.player === 'user'
      ? (GameState.player = 'computer')
      : (GameState.player = 'user');
  }
